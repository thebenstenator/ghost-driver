...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // CRITICAL — the ease MUST be time-bounded (seMaxTime): the cap is BELOW your speed while far, so
    // a cop chasing a fleeing player can never reach seNear to end it on distance → the cap, and the
...
searchSpeed: ${t.searchSpeed}, searchDepth: ${t.searchDepth}, searchMaxDepth: ${t.searchMaxDepth}, coverageTTL: ${t.coverageTTL}, searchDirBias: ${t.searchDirBias}, searchDwell: ${t.searchDwell}, searchStall: ${t.searchStall}`);
  ...
  update(_time, delta) {
    ...
    for (const cop of this.cops) {
      ...
      // Patrol sight-edge rubber band (raises the cap toward player top so straight-lining can't
      // shed a patrol). MAX'd over the boosts above, so it only ever helps. See _applyPatrolBand.
      this._applyPatrolBand(
        cop,
        Phaser.Math.Distance.Between(cop.sprite.x, cop.sprite.y, px, py),
        state === PursuitState.ACTIVE,
      );
      ...
      if (cop._spawnEase) {
        cop._spawnEaseT = (cop._spawnEaseT || 0) + delta / 1000;
        const d = Phaser.Math.Distance.Between(cop.sprite.x, cop.sprite.y, px, py);
        if (d <= this.seNear || cop._spawnEaseT >= this.seMaxTime) {
          cop._spawnEase = false; // engaged (got close OR the ease timed out) — hand off to normal chasing
        } else {
